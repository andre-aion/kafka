...
class PythonCassandra:
    ...
    def insert_data_block(self, message):
        ...
        batch.add(insert_sql, message)
        self.session.execute(batch)
        self.log.info('Transaction Batch Insert Completed')
...
pc.create_table_transaction()


KAFKA_TOPIC = 'aionv4_transaction'
consumer = KafkaConsumer(KAFKA_TOPIC,
                         auto_offset_reset='earliest',api_version=(0, 10, 1))
try:
    for message in consumer:
        data = json.loads(message.value.decode('utf-8'))
        print("consumer_transaction:{}".format(data))
        pc.insert_data_transaction(data)
